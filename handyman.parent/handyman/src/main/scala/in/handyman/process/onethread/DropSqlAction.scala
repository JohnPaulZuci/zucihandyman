...
class DropSqlAction extends in.handyman.command.Action with LazyLogging {
  val detailMap = new java.util.HashMap[String, String]
  val auditMarker = "DROPSQL";
  val aMarker = MarkerFactory.getMarker(auditMarker);

  def execute(context: Context, action: Action, actionId:Integer): Context = {
    val dropSqlAsIs = action.asInstanceOf[in.handyman.dsl.DropSql]
    val transform: in.handyman.dsl.DropSql = CommandProxy.createProxy(dropSqlAsIs, classOf[in.handyman.dsl.DropSql], context)
    		...
    		val sqlWithoutQuotes = iter.next.replaceAll("\"", "")
    				...
    				if (!sql.trim.isEmpty()) {
    				  val stringreplace:String = sql.replaceAll(" ", "")
    					val firstFourChr:String = stringreplace.substring(0, 4)
    							if(firstFourChr.equalsIgnoreCase("drop"))
    							{
    								...
    								throw new Exception("The expected operating must be drop but we found => "+sql);
    							}
    				}
    		}
    	}
