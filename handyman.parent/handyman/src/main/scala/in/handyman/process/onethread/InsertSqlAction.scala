...
class InsertSqlAction extends in.handyman.command.Action with LazyLogging {
  val detailMap = new java.util.HashMap[String, String]
  val auditMarker = "INSERTSQL";
  val aMarker = MarkerFactory.getMarker(auditMarker);

  def execute(context: Context, action: Action, actionId:Integer): Context = {
    val insertSqlAsIs = action.asInstanceOf[in.handyman.dsl.InsertSql]
    val insertSql: in.handyman.dsl.InsertSql = CommandProxy.createProxy(insertSqlAsIs, classOf[in.handyman.dsl.InsertSql], context)

    val dbSrc = insertSql.getOn
    val name = insertSql.getName
    val id = context.getValue("process-id")
    val sqlList = insertSql.getValue
    				...
    				if (!sql.trim.isEmpty()) {
    				   val stringreplace:String = sql.replaceAll(" ", "")
    					val firstSixChr:String = stringreplace.substring(0, 6)
    							if(firstSixChr.equalsIgnoreCase("insert"))
    							{
    								logger.info(aMarker, "Transform id#{}, executing script {}", id, sql.trim)
    								val statementId = AuditService.insertStatementAudit(actionId, "insertSql->"+name, context.getValue("process-name"))
    								try {
    								...
    								throw new Exception("The expected operating must be insert but we found => "+sql);
    							}
    				}
    		}
    	}
  ...
  def executeIf(context: Context, action: Action): Boolean = {
    ...
    val name = insertSql.getName
    val id = context.getValue("process-id")
    val expression = insertSql.getCondition
    try {
