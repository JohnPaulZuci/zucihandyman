...
class HiveAction extends in.handyman.command.Action with LazyLogging {
  val detailMap = new java.util.HashMap[String, String]
  val auditMarker = "HIVE";
  val aMarker = MarkerFactory.getMarker(auditMarker);

  def execute(context: Context, action: Action, actionId:Integer): Context = {
    val hiveAsIs = action.asInstanceOf[in.handyman.dsl.Hive]
    val hive: in.handyman.dsl.Hive = CommandProxy.createProxy(hiveAsIs, classOf[in.handyman.dsl.Hive], context)

    val dbSrc = hive.getOn
    val name = hive.getName
    val id = context.getValue("process-id")
    val sqlList = hive.getValue
  ...
  def executeIf(context: Context, action: Action): Boolean = {
    ...
    val name = hive.getName
    val id = context.getValue("process-id")
    val expression = hive.getCondition
    try {
