...
class DeleteSqlAction extends in.handyman.command.Action with LazyLogging {
  val detailMap = new java.util.HashMap[String, String]
  val auditMarker = "DELTESQL";
  val aMarker = MarkerFactory.getMarker(auditMarker);

  def execute(context: Context, action: Action, actionId:Integer): Context = {
    val deleteAsIs = action.asInstanceOf[in.handyman.dsl.DeleteSql]
    val deleteSql: in.handyman.dsl.DeleteSql = CommandProxy.createProxy(deleteAsIs, classOf[in.handyman.dsl.DeleteSql], context)

    val dbSrc = deleteSql.getOn
    val name = deleteSql.getName
    val id = context.getValue("process-id")
    val sqlList = deleteSql.getValue
    		...
    		val sqlWithoutQuotes = iter.next.replaceAll("\"", "")
    				...
    				if (!sql.trim.isEmpty()) {
    					val stringreplace:String = sql.replaceAll(" ", "")
    							...
    							if(firstSixChr.equalsIgnoreCase("delete"))
    							{
    								logger.info(aMarker, "Transform id#{}, executing script {}", id, sql.trim)
    								val statementId = AuditService.insertStatementAudit(actionId, "deleteSql->"+name, context.getValue("process-name"))
    								try {
    								...
    								throw new Exception("The expected operating must be delete but we found => "+sql);
  ...
  def executeIf(context: Context, action: Action): Boolean = {
    ...
    val name = deleteSql.getName
    val id = context.getValue("process-id")
    val expression = deleteSql.getCondition
    try {
